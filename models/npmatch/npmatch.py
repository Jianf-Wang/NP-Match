...
class NPMatch:
    ...
    def train(self, args, logger=None):
        ...
        # EMA Init
        self.model.train()
        self.np_head_model.train()
        self.ema = EMA(self.model, self.ema_m)
        self.np_ema = EMA(self.np_head_model, self.ema_m)
        self.ema.register()
        self.np_ema.register()
    ...
    def evaluate(self, eval_loader=None, args=None):
        ...
        self.np_head_model.eval()
        self.np_ema.apply_shadow()
        ...
        for _, x, y in eval_loader:
            ...
            loss = F.cross_entropy(logits_out, y, reduction='mean')
            y_true.extend(y.cpu().tolist())
            y_pred.extend(torch.max(logits_out, dim=-1)[1].cpu().tolist())
    ...
    def save_model(self, save_name, save_path):
        ...
        ema_np_head_model = self.np_head_model.state_dict()
        self.ema.restore()
        self.np_ema.restore()
        self.model.train()
        self.np_head_model.train()
        ...
        torch.save({'model': self.model.state_dict(),
                    ...
                    'it': self.it,
                    'ema_model': ema_model,
                    'ema_np_head_model': ema_np_head_model},
                   save_filename)
    ...
    def load_model(self, load_path):
        ...
        self.np_head_model.load_state_dict(checkpoint['np_head_model'])
        self.ema_np_head_model = deepcopy(self.np_head_model)
        self.ema_model.load_state_dict(checkpoint['ema_model'])
        self.ema_np_head_model.load_state_dict(checkpoint['ema_np_head_model'])
